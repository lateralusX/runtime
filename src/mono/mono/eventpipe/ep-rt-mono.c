	...
	bool *runtime_invoke_frame)
{
	...
	case FRAME_TYPE_JIT_ENTRY:
		...
		if (_ep_rt_mono_runtime_helper_compile_method && *top_frame)
			ep_stack_contents_append ((EventPipeStackContents *)data, (uintptr_t)((uint8_t*)_ep_rt_mono_runtime_helper_compile_method), _ep_rt_mono_runtime_helper_compile_method);
		*top_frame = false;
		return FALSE;
		...
		if (frame->ji) {
			*async_frame |= frame->ji->async;
			...
			if (method && m_method_is_wrapper (method)) {
				...
				if (in_safe_point_frame ((EventPipeStackContents *)data, wrapper)) {
					*safe_point_frame = true;
				}else if (in_runtime_invoke_frame ((EventPipeStackContents *)data, wrapper)) {
					*runtime_invoke_frame = true;
				} else if (_ep_rt_mono_monitor_enter_method && in_monitor_enter_frame (wrapper)) {
					ep_stack_contents_append ((EventPipeStackContents *)data, (uintptr_t)((uint8_t*)_ep_rt_mono_monitor_enter_method), _ep_rt_mono_monitor_enter_method);
				} else if (_ep_rt_mono_monitor_enter_v4_method && in_monitor_enter_v4_frame (wrapper)) {
					ep_stack_contents_append ((EventPipeStackContents *)data, (uintptr_t)((uint8_t*)_ep_rt_mono_monitor_enter_v4_method), _ep_rt_mono_monitor_enter_v4_method);
...
eventpipe_walk_managed_stack_for_thread_func (
	...
	void *data)
{
	EventPipeStackWalkData *stack_walk_data = (EventPipeStackWalkData *)data;
	return eventpipe_walk_managed_stack_for_thread (frame, ctx, stack_walk_data->stack_contents, &stack_walk_data->top_frame, &stack_walk_data->async_frame, &stack_walk_data->safe_point_frame, &stack_walk_data->runtime_invoke_frame);
}
	...
	void *data)
{
	...
	return eventpipe_walk_managed_stack_for_thread (frame, ctx, &sample_data->stack_contents, &sample_data->top_frame, &sample_data->async_frame, &sample_data->safe_point_frame, &sample_data->runtime_invoke_frame);
}
		...
		if (!mono_thread_info_is_running (thread_info)) {
			...
			if (thread_state->valid) {
				if (sampled_thread_count < _ep_rt_mono_max_sampled_thread_count) {
					...
					data->runtime_invoke_frame = false;
					...
					if (data->payload_data == EP_SAMPLE_PROFILER_SAMPLE_TYPE_EXTERNAL && (data->safe_point_frame || data->runtime_invoke_frame)) {
