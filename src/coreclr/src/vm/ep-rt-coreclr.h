...
#define EP_RT_DEFINE_ARRAY_PREFIX(prefix_name, array_name, array_type, iterator_type, item_type) \
	...
	static inline void EP_RT_BUILD_TYPE_FUNC_NAME(prefix_name, array_name, clear) (array_type *ep_array, void (*callback)(void *)) { \
		EP_ASSERT (ep_array->array != NULL); \
		ep_array->array->Shrink (0); \
	} \
...
{
	// TODO: Implement. Need to change to platform thread in places where we use EventPipeThread.
	return true;
}
